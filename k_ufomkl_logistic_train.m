function model = k_ufomkl_logistic_train(K, Y, model, options)
% K_UFOMKL_LOGISTIC_TRAIN Ultra Fast Optimization for Multi Kernel Learning
%
%    MODEL = K_UFOMKL_LOGISTIC_TRAIN(K,Y,MODEL) trains a sparse Multi
%    Kernel binary classifier using Ultra Fast Optimization algorithm,
%    using precomputed kernels. The loss function is the logistic loss.
%
%    Inputs:
...
for epoch = 1:model.T
    ...
    for i = 1:n
        ...
        % the exponential could overflow to infinity
        if -yi*val_f>=87
            loss = -yi*val_f;
            eta = -yi;
        else
            val_exp = exp(-yi*val_f);
       	    loss = log(1+val_exp);
            eta = -yi*val_exp/(1+val_exp);
        end
        ...
        if loss>0
            ...
            beta(idxs_subgrad) = beta(idxs_subgrad)-eta;
            sqnorms = sqnorms - 2*eta*preds + Kii(:)';

            n_update = n_update+1;
            
            % update predmat
            predmat = predmat - squeeze(eta*K(idxs_subgrad,:,:));
